...
metal2.add_feature(Line(Orientation.VERTICAL, thickness=20, position=200))
...
bigpillar = metal1.features[3]
bigpillar.y += 3
img_updated = layout.rerender_feature(bigpillar).copy()

vline_m2 = metal2.features[3]
vline_m2.position += 30
img_updated = layout.rerender_feature(vline_m2).copy()


diff = img_updated.astype(float) - img_initial.astype(float)
...
axes[1].set_title("Various shifts")
